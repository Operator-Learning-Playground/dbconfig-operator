...
type DbConfigLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.DbConfig, err error)
...
// List lists all DbConfigs in the indexer.
func (s *dbConfigLister) List(selector labels.Selector) (ret []*v1alpha1.DbConfig, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.DbConfig))
	})
	return ret, err
}
...
type DbConfigNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha1.DbConfig, error)
...
// List lists all DbConfigs in the indexer for a given namespace.
func (s dbConfigNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.DbConfig, err error) {
...
// Get retrieves the DbConfig from the indexer for a given namespace and name.
func (s dbConfigNamespaceLister) Get(name string) (*v1alpha1.DbConfig, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("dbconfig"), name)
	}
	return obj.(*v1alpha1.DbConfig), nil
}
